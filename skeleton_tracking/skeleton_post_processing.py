...
class SkeletonDetection(PostProcessing):
    def __init__(self):
        ...
        self.internal_node.declare_parameter(
            'skeleton_detection_node.debug_topic', True
        )
        self.internal_node.declare_parameter(
        ...
        self.internal_node.declare_parameter(
            'skeleton_detection_node.roi_half_size', 0
        )

        self.debug = self.internal_node.get_parameter(
            'skeleton_detection_node.debug_topic'
        ).value
        self.min_detection_confidence = self.internal_node.get_parameter(
            'skeleton_detection_node.min_detection_confidence'
        ).value
        self.min_tracking_confidence = self.internal_node.get_parameter(
            'skeleton_detection_node.min_tracking_confidence'
        ).value
        self.roi_half_size = self.internal_node.get_parameter(
            'skeleton_detection_node.roi_half_size'
        ).value

        if self.roi_half_size < 0:
            self.internal_node.get_logger().warning(
                'ROI half size must be greater than 0'
            )
        ...
        for segment in mp.solutions.pose.POSE_CONNECTIONS:
            ...
            if idx1 in indexes and idx2 in indexes:
                ...
                skeleton.points.append(
                    Point(x=float(x1), y=float(y1), z=float(z1))
                )
                skeleton.points.append(
                    Point(x=float(x2), y=float(y2), z=float(z2))
                )
        return skeleton
    ...
    def process_frames(self, color_frame, distance_frame):
        ...
        for keypoint_id, keypoint in enumerate(keypoints):
            if not (
                ...
                continue
            x, y = de_normalize_keypoint(
                keypoint, self.camera_info.width, self.camera_info.height
            )

            y_min = clamp(
                y - self.roi_half_size, 0, self.camera_info.height - 1
            )
            y_max = clamp(
                y + 1 + self.roi_half_size, 0, self.camera_info.height - 1
            )  # +1 to include the last pixel
            x_min = clamp(x - self.roi_half_size, 0, self.camera_info.width - 1)
            x_max = clamp(
                x + 1 + self.roi_half_size, 0, self.camera_info.width - 1
            )  # +1 to include the last pixel
            ...
            valid_values = roi_distance[
                (roi_distance > 0) & np.isfinite(roi_distance)
            ]
        ...
        if self.debug:
            ...
            mp.solutions.drawing_utils.draw_landmarks(
                color_frame_with_detection,
                result.pose_landmarks,
                mp.solutions.pose.POSE_CONNECTIONS,
            )

            self.debug_publisher.publish(
                self.cv_bridge.cv2_to_imgmsg(color_frame_with_detection)
            )
    ...
    def is_not_person(self, indexes_present, landmark_list):
        ...
        if (
            mp_pose.PoseLandmark.LEFT_SHOULDER.value in indexes_present
            and mp_pose.PoseLandmark.LEFT_HIP.value in indexes_present
        ):

            idx_left_shoulder = indexes_present.index(
                mp_pose.PoseLandmark.LEFT_SHOULDER.value
            )
            idx_left_hip = indexes_present.index(
                mp_pose.PoseLandmark.LEFT_HIP.value
            )
            left_bust = np.linalg.norm(
                np.array(landmark_list[idx_left_shoulder])
        ...
        if (
            mp_pose.PoseLandmark.RIGHT_SHOULDER.value in indexes_present
            and mp_pose.PoseLandmark.RIGHT_HIP.value in indexes_present
        ):

            idx_right_shoulder = indexes_present.index(
                mp_pose.PoseLandmark.RIGHT_SHOULDER.value
            )
            idx_right_hip = indexes_present.index(
                mp_pose.PoseLandmark.RIGHT_HIP.value
            )
            right_bust = np.linalg.norm(
                np.array(landmark_list[idx_right_shoulder])
