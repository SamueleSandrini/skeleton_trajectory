...
class SkeletonFilterNode(Node):
    ...
    def load_custom_publishers_modules(self, custom_publishers):
        for publisher in custom_publishers:
            ...
            publisher_full_module_name = self.get_parameter(
                f'{publisher}.module').get_parameter_value().string_value
            ...
            try:
                ...
                if not issubclass(custom_pub_class, CustomPublisherBase):
                    self.get_logger().warning(
                        f'The custom publisher class \
                        {custom_pub_class.__name__} \
                        is not a subclass of CustomPublisherBase.')
                    continue
    ...
    def callback_keypoint(self, keypoints_msg: MarkerArray):
        ...
        skeleton = build_skeleton_topology_msg(keypoints,
                                               header)
        self.skeleton_topology_publisher.publish(skeleton)

        marker_array = build_marker_array_msg(keypoints,
                                              header)
